...
const tracks = [
  ...
  { id: 'ai', title: 'AI Skills', icon: Brain, color: '#23b26d', helper: 'Chib', description: 'Use AI as a small daily helper.', lessons: [
...
const storageKey = 'nannie-academy-progress-v4';
...
function App() {
  ...
  return <main className="academy-shell">
    ...
    <section className="academy-main">
      <header className="topbar"><div><p className="place-pill">Daily quests made for Nannie</p><h1>Nannie Academy</h1><p>Tiny lessons, cute helpers, steady progress.</p></div><button className="icon-action" onClick={reset} title="Reset progress"><RotateCcw size={18} /></button></header>
      ...
      <section className="content-grid"><article className="lesson-map"><div className="card-head"><div><h2>{selected.title}</h2><p>{selected.description}</p><p className="helper-line">Helper: {selected.helper}</p></div><span className="xp-pill">{selected.lessons.length} quests</span></div>{selected.lessons.map((lesson, index) => { const isDone = done.includes(lesson.id); return <button key={lesson.id} className={isDone ? 'lesson-node complete' : 'lesson-node'} onClick={() => completeQuest(lesson.id)} disabled={isDone}><span>{isDone ? <Check size={20} /> : index + 1}</span><div><strong>{lesson.title}</strong><p>{lesson.task}</p></div><ChevronRight size={18} /></button>; })}</article>
        <aside className="side-stack"><article className="voice-card"><div className="mini-mascots"><div className="tiny-bear" /><div className="tiny-kitty" /></div><h2>Voice Classroom</h2><p>Practice real conversations with her private GPT, then send progress back to Chib.</p><a href="https://chatgpt.com/gpts" target="_blank" rel="noreferrer">Open Voice</a><small>Next: replace this with Nannie’s private GPT link.</small></article><article className="rewards-card"><div className="card-head compact"><h2>Rewards</h2><p>Badges unlock as she practices</p></div><div className="badge-grid">{badges.map((badge) => { const Icon = badge.icon; const unlocked = done.length >= badge.need; return <div key={badge.id} className={unlocked ? 'badge unlocked' : 'badge'}><Icon size={18} /><span>{badge.title}</span></div>; })}</div></article><article className="notes-card"><h2>Mentor Notes</h2><p>Separate learner memory</p><ul><li>Keep lessons small and encouraging</li><li>Praise effort before correction</li><li>Use personal examples inside exercises, not as labels</li><li>Memory should track progress, not private diary details</li></ul><p className="next-level">{nextLevelXp} XP until next level</p></article></aside>
      </section>
