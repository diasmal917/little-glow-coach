...
function buildWeeklySummary(reflections, done, lessons, xp, level) {
  ...
  return [
    ...
    '',
    'Recent reflections / post ideas:',
    recent.length ? recent.map((item) => `- ${item.lessonTitle}: ${item.answer}`).join('\n') : '- None yet',
    '',
    'Chib, please review her progress, praise effort first, improve her English/content ideas, and choose the next best quest.',
  ].join('\n');
}
